...
public class MenuPerfilDAO extends DataBaseDAO{
    ...
    public ArrayList<Menu> menusVinculadosPorPerfil(int idPerfil){
        ...
        String sql = "SELECT m.* FROM menu_perfil as mp, menu as m WHERE mp.Menu_idMenu = m.idMenu AND mp.Perfil_idPerfil=?";
    ...
    public ArrayList<Menu> menusNaoVinculadosPorPerfil(int idPerfil){
        ...
        ArrayList<Menu> lista = new ArrayList<>();
        String sql = "SELECT * FROM menu m WHERE idMenu NOT IN (SELECT Menu_idMenu FROM menu_perfil WHERE Perfil_idPerfil=?)";
        try{
            //abre a conexao com o banco
            this.conectar();
            //Usando try-with-resources para PreparedStatement e ResultSet
